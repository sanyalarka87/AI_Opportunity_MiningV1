...
export const STUB_RESPONSES = {
  default:
    'This is a stub response from the Executive Insight Agent. In production, this would be generated by the multi-agent AI layer based on your prompt and current dashboard context.',
  'Show monthly revenue and cost trends':
    'Monthly revenue has remained stable at approximately $35M, while medical cost shows a 2.1% increase over the last three months. PMPM is trending at $336. Recommendation: monitor high-cost segments.',
  'Rank LOBs based on revenue and cost performance':
    'Ranking by margin: 1) Commercial (MLR 85%), 2) Medicare (MLR 90%), 3) Medicaid (MLR 94%). Commercial shows the strongest performance with lowest MLR.',
  'Identify top 5 margin recovery opportunities':
    'Top 5: (1) Prior auth automation — $2.1M savings, (2) Care gap closure — $1.8M, (3) ER diversion — $1.5M, (4) Pharmacy utilization — $1.2M, (5) Inpatient length of stay — $0.9M.',
  'Summarize margin improvement story for board presentation':
    'Year-over-year MLR improved from 91% to 90.2%. We have 47 identified opportunities with $12.5M potential savings; 8 approved and in execution. Key focus: prior authorization and care gaps.',
...
export function buildOpportunityQnAResponse(opp, messageTrimmed) {
...
export function parseRealizationRateFromScenario(scenario) {
  if (!scenario || typeof scenario !== 'string') return null;
  const trimmed = scenario.trim();
  const match =
    trimmed.match(/(?:realization\s+rate\s+(?:is|at|to|of)\s*)?(\d+)\s*%?/i) ||
...
export function parseMLRReductionFromScenario(scenario) {
  if (!scenario || typeof scenario !== 'string') return null;
  const trimmed = scenario.trim();
  const match =
    trimmed.match(/mlr\s+from\s+(\d+)\s*%\s*to\s+(\d+)\s*%/i) ||
...
export function plannerAnalyze(message) {
